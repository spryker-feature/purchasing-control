...
class BudgetUpdater implements BudgetUpdaterInterface
{
    ...
    public function updateBudgetCollection(BudgetCollectionRequestTransfer $budgetCollectionRequestTransfer): BudgetCollectionResponseTransfer
    {
        ...
        if ($validBudgetTransfers->count()) {
            $validBudgetTransfers = $this->getTransactionHandler()->handleTransaction(
                fn () => $this->executeUpdateBudgetCollectionTransaction($validBudgetTransfers),
            );
        }
     ...
     */
    protected function executeUpdateBudgetCollectionTransaction(ArrayObject $budgetTransfers): ArrayObject
    {
        $persistedTransfers = new ArrayObject();

        foreach ($budgetTransfers as $index => $budgetTransfer) {
            $persistedTransfers->offsetSet($index, $this->entityManager->updateBudget($budgetTransfer));
        }
