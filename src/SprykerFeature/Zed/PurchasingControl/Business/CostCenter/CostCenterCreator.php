...
class CostCenterCreator implements CostCenterCreatorInterface
{
    ...
    public function createCostCenterCollection(CostCenterCollectionRequestTransfer $costCenterCollectionRequestTransfer): CostCenterCollectionResponseTransfer
    {
        ...
        if ($validCostCenterTransfers->count()) {
            $validCostCenterTransfers = $this->getTransactionHandler()->handleTransaction(
                fn () => $this->executeCreateCostCenterCollectionTransaction($validCostCenterTransfers),
            );
        }
     ...
     */
    protected function executeCreateCostCenterCollectionTransaction(ArrayObject $costCenterTransfers): ArrayObject
    {
        $persistedTransfers = new ArrayObject();

        foreach ($costCenterTransfers as $index => $costCenterTransfer) {
            $persistedTransfers->offsetSet($index, $this->entityManager->createCostCenter($costCenterTransfer));
        }
