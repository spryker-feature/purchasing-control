...
class PurchasingControlMapper
{
            ...
            if ($companyBusinessUnitEntity !== null) {
                $costCenterTransfer->addCompanyBusinessUnit(
                    (new CompanyBusinessUnitTransfer())
                        ->setIdCompanyBusinessUnit($companyBusinessUnitEntity->getIdCompanyBusinessUnit())
                        ->setName($companyBusinessUnitEntity->getName()),
                );
            }
        }
    ...
    {
        $costCenterEntity->fromArray($costCenterTransfer->modifiedToArray());
    ...
    public function mapBudgetEntityToTransfer(SpyBudget $budgetEntity, BudgetTransfer $budgetTransfer): BudgetTransfer
    {
        ...
        $budgetTransfer->setAmount((int)$budgetTransfer->getAmount());
        $budgetTransfer->setIdCostCenter($budgetEntity->getFkCostCenter());

        $budgetArray = $budgetEntity->toArray();

        if (isset($budgetArray[static::COL_CONSUMED_AMOUNT])) {
            $consumedAmount = (int)$budgetArray[static::COL_CONSUMED_AMOUNT];
    ...
    public function mapBudgetConsumptionEntityToTransfer(
        SpyBudgetConsumption $budgetConsumptionEntity,
        BudgetConsumptionTransfer $budgetConsumptionTransfer
    ): BudgetConsumptionTransfer {
        $budgetConsumptionTransfer->fromArray($budgetConsumptionEntity->toArray(), true);
        $budgetConsumptionTransfer->setAmount((int)$budgetConsumptionTransfer->getAmount());
        $budgetConsumptionTransfer->setIdBudget($budgetConsumptionEntity->getFkBudget());
        $budgetConsumptionTransfer->setIdSalesOrder($budgetConsumptionEntity->getFkSalesOrder());
    ...
    public function mapBudgetConsumptionTransferToEntity(
        ...
        $budgetConsumptionEntity->setAmount((string)$budgetConsumptionTransfer->getAmountOrFail());
